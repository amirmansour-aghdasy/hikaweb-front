    ...
    return (
        <form onSubmit={() => {}} className="w-full flex flex-col gap-3 md:gap-4 p-3 md:p-3.5 rounded-2xl border border-teal-500 shadow-md shadow-teal-500" data-aos="fade-right" data-aos-delay="250">
            ...
            <Listbox value={selected} onChange={setSelected}>
                <ListboxButton
                    className={clsx(
                        "relative block w-full rounded-md bg-slate-50 py-3 pl-8 pr-3 text-right text-slate-500",
                ...
                >
                    {people.map((person) => (
                        <ListboxOption key={person.name} value={person} className="group flex cursor-default items-center gap-2 rounded-lg px-3 py-1.5 select-none data-focus:bg-white/10">
                            <HiCheck className="invisible size-4 fill-slate-500 group-data-selected:visible" />
