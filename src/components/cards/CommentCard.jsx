...
const CommentCard = ({ comment: { writer, text, position, thumbnail } }) => {
    return (
        <div className="flex flex-col gap-y-2 sm:gap-y-3.5 rounded-2xl border border-teal-500 p-3">
            <div className="flex items-center gap-x-2">
                {thumbnail ? (
                    <Image src={thumbnail} alt={writer} title={writer} sizes="100vw" width={0} height={0} className="rounded-full border border-slate-100 w-10 h-10 md:w-12 md:h-12 shadow-md" />
                ) : (
            ...
            <p className="text-xs sm:text-sm text-slate-500 leading-normal md:leading-6 line-clamp-3">{text}</p>
