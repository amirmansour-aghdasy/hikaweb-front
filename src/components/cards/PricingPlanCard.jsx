...
const PricingPlanCard = ({ plan: { title, value, subTitle, features } }) => {
    return (
        <div className="w-full h-full rounded-2xl flex flex-col">
            <div className="w-full border border-teal-600 flex flex-col justify-center items-center gap-1.5 rounded-t-2xl p-3.5 md:p-7 bg-[#F6F6F6]">
                <h3 className="font-bold text-teal-700 text-xl md:text-2xl">{title}</h3>
                <h4 className="text-base md:text-lg text-slate-500">{value}</h4>
                <p className="text-sm md:text-base text-teal-500">{subTitle}</p>
            </div>
            <ul className="w-full h-full flex flex-col gap-2 pr-9 pt-5 pb-5 cursor-default">
            ...
            <div className="w-full flex flex-col justify-center items-center gap-4 bg-teal-500 rounded-b-2xl p-3.5 md:p-6">
                ...
                <Link href="" className="w-8/12 text-center text-sm md:text-lg text-teal-900 rounded-md py-1.5 md:py-0.5 bg-white">
                    درخواست مشاوره
                </Link>
            </div>
        </div>
